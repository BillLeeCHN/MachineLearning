...
class LinearRegressor:
    ...
    def train(self, data):
        for i in range(self.num_iteration):
            gradient_m, gradient_b = self.computer_gradient(data)
            self.m -= gradient_m * self.learning_rate
            self.b -= gradient_b * self.learning_rate

    def computer_gradient(self, data):
        total_gradient_m = 0
        total_gradient_b = 0
...
def run():
    ...
    print("Before training, error is {0}".format(model.computerMSE(data)))
    model.train(data)
    print("After training, eoor is {0}".format(model.computerMSE(data)))
